...
<div class="row mb-3">
    <div class="col-sm-4 mb-3" v-if="isDivision">
        @php /** @var string $errors */
            $error_class = $errors->has('division_id') ? 'parsley-error ' : ''; @endphp
        <label for="division_id" class="form-label">@lang('user.label_division_id')</label>
        <div class="form-group">
            {{ Form::select('division_id', $divisions,null, ['class' => $error_class.'form-control select2vue division_id', 'v-model' => 'division_id', 'placeholder'=>trans('user.label_select_one'), 'onchange' => 'SelectChange("'.route('get_districts').'","district_id",this)']) }}
    ...
    <div class="col-sm-4 mb-3">
        ...
        <label for="district_id" class="form-label">@lang('user.label_district_id')</label>
        <div class="form-group">
            {{ Form::select('district_id', $districts,null, ['class' => $error_class.'form-control select2vue district_id', 'id' => 'district_id', 'placeholder'=>trans('user.label_select_one'), 'onchange' => 'SelectChange("'.route('get_fire_stations').'","fire_station_id",this)']) }}
    ...
    <div class="col-sm-4 mb-3">
        ...
        <label for="fire_station_id" class="form-label">@lang('user.label_fire_station_id')</label>
        <div class="form-group">
            {{ Form::select('fire_station_id', $fire_stations,null, ['class' => $error_class.'form-control select2vue fire_station_id', 'id' => 'fire_station_id', 'placeholder'=>trans('user.label_select_one')]) }}
    ...
    <div class="col-sm-4 mb-3">
        @php /** @var string $errors */
            $error_class = $errors->has('employer_id') ? 'parsley-error ' : ''; @endphp
        <label for="employer_id" class="form-label">@lang('user.label_employer_id')</label>
        <div class="form-group">
            {{ Form::select('employer_id', $employers,null, ['class' => $error_class.'form-control select2vue employer_id', 'id' => 'employer_id', 'placeholder'=>trans('user.label_select_one')]) }}
    ...
    <div class="col-sm-4 mb-3">
        ...
        <label for="workshop_id" class="form-label">@lang('user.label_workshop_id')</label>
        <div class="form-group">
            {{ Form::select('workshop_id', $workshops,null, ['class' => $error_class.'form-control select2vue workshop_id', 'id' => 'workshop_id', 'placeholder'=>trans('user.label_select_one')]) }}
...
@section('script')
    ...
    <script>
        ...
        function GetFireStationFromDistrict() {
            ...
            $.ajax({
                url: '{{route('get_fire_station_from_district')}}',
                type: 'POST',
                dataType: 'JSON',
                cache: false,
                data:{
                ...
                success: function (response) {
                    let fire_station_id = $('#fire_station_id');

                    //success data
                    fire_station_id.empty();
                    fire_station_id.append("<option value=''>{{trans('user.label_select_one')}}</option>");
                    $.each(response, function (index, subcatObj) {
                        fire_station_id.append("<option value='" + subcatObj.id + "'>" + subcatObj.bn_name + "</option>");
                    });
                },
                error: function (response, error) {
                    console.log(error)

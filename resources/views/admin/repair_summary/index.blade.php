...
@section('title') @lang('common.index',['model' => trans('repair_summary.repair_summary')]) @endsection
...
@section('thead')
    <tr>
        ...
        <th width="20%">@lang('common.action')</th20</tr>
@endsection

@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('common.index',['model' => trans('repair_summary.repair_summary')])@endslot
        @slot('create_button')
            <a href="{{route('repair_summary.create')}}" class="btn btn-primary btn-sm waves-effect waves-light">
                <i class="fa fa-plus-circle"></i> @lang('common.create',['model' => trans('repair_summary.repair_summary')])
            </a>
        @endslot
    @endcomponent
    ...
    <ul class="nav nav-tabs" id="Tab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active text-primary" id="all_btn_tab" data-bs-toggle="tab" data-bs-target="#all" type="button" role="tab" aria-controls="all" aria-selected="true">@lang('common.index_list',['model' => trans('repair_summary.repair_summary')])</button>
        </li>
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            <li class="nav-item" role="presentation">
                <button class="nav-link text-primary" id="deleted_btn_tab" data-bs-toggle="tab" data-bs-target="#deleted_list" type="button" role="tab" aria-controls="deleted_list" aria-selected="false">@lang('common.deleted_list',['model' => trans('repair_summary.repair_summary')])</button>
            </li>
        @endif
    </ul>
    ...
    <script type="text/javascript">
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            $('#deleted_list_datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('repair_summary.deleted_list') }}',
                    type: 'get',
                    dataType: 'JSON',
                    cache: true,
                },

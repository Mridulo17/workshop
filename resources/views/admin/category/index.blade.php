...
@section('title') @lang('common.index',['model' => trans('category.category')]) @endsection
...
@section('thead')
    <tr>
        ...
        <th>@lang('common.name',['model' => trans('category.category')])</th>
        <th>@lang('common.bn_name',['model' => trans('category.category')])</th>
        <th width="5%">@lang('common.status',['model' => trans('category.category')])</th>
        <th width="12%">@lang('common.action',['model' => trans('category.category')])</th>
    </tr>
@endsection

@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('common.index',['model' => trans('category.category')])@endslot
        @slot('create_button')
            <a href="{{route('category.create')}}" class="btn btn-primary btn-sm waves-effect waves-light modal_lg_button">
                <i class="fa fa-plus-circle"></i> @lang('common.create',['model' => trans('category.category')])
            </a>
        @endslot
    @endcomponent
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            ...
            ajax: {
                url: '{{ route('category.deleted_list') }}',
                type: 'get',
                dataType: 'JSON',
                cache: false,
            },

...
@section('title') @lang('common.index',['model' => trans('battery_record.battery_record')]) @endsection
...
@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('common.index',['model' => trans('battery_record.battery_record')])@endslot
        @slot('create_button')
            <a href="{{route('battery_record.create')}}" class="btn btn-primary btn-sm waves-effect waves-light">
                <i class="fa fa-plus-circle"></i> @lang('common.create',['model' => trans('battery_record.battery_record')])
            </a>
        @endslot
    @endcomponent
    ...
    <ul class="nav nav-tabs" id="Tab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active text-primary" id="all_btn_tab" data-bs-toggle="tab" data-bs-target="#all" type="button" role="tab" aria-controls="all" aria-selected="true">@lang('common.index_list',['model' => trans('battery_record.battery_record')])</button>
        </li>
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            <li class="nav-item" role="presentation">
                <button class="nav-link text-primary" id="deleted_btn_tab" data-bs-toggle="tab" data-bs-target="#deleted_list" type="button" role="tab" aria-controls="deleted_list" aria-selected="false">@lang('common.deleted_list',['model' => trans('battery_record.battery_record')])</button>
            </li>
        @endif
    </ul>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('#datatable').DataTable({
            ...
            ajax: {
                url: '{{ route('battery_record.index') }}',
                type: 'get',
                dataType: 'JSON',
                cache: true,
            },
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            $('#deleted_list_datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('battery_record.deleted_list') }}',
                    type: 'get',
                    dataType: 'JSON',
                    cache: true,
                },

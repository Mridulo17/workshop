                                            ...
                                            <ul class="sub-menu" aria-expanded="true">
                                                @foreach($sub_menu->children as $child_menu)
                                                    ...
                                                    <li class="{{ (str_replace('-','_',Request::segment(2)) == explode('.',$child_menu->route_name)[0] && (str_replace('-','_',Request::segment(2)) != null))?'mm-active':''}}">
                                                        <a href="{{$menu_element['menu_link']}}" class="{{$menu_element['has_arrow']}} waves-effect" key="t-vertical"><i class="{{$menu_element['menu_icon']}}"></i>{{\App\Helpers\MenuHelper::menuName($child_menu)}}</a>

                                                        {{-------3rd child-----start-------}}
                                                        @if(count($child_menu->children) > 0)
                                                            <ul class="sub-menu" aria-expanded="true">
                                                                @foreach($child_menu->children as $sub_child_menu)
                                                                    @php
                                                                        $menu_element = App\Helpers\MenuHelper::MenuElement($sub_child_menu);
                                                                    @endphp
                                                                        <li class="{{ (str_replace('-','_',Request::segment(2)) == explode('.',$sub_child_menu->route_name)[0] && (str_replace('-','_',Request::segment(2))) != null)?'mm-active':''}}">
                                                                            <a href="{{$menu_element['menu_link']}}" key="t-vertical">  <i class="{{$menu_element['menu_icon']}}"></i> {{\App\Helpers\MenuHelper::menuName($sub_child_menu)}}</a>
                                                                        </li>
                                                                @endforeach
                                                            </ul>
                                                        @endif

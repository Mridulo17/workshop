...
@section('title') @lang('variant_type.index') @endsection
...
@section('thead')
    <tr>
        <th width="5%">#</th>
        <th>@lang('variant_type.name')</th>
        <th>@lang('variant_type.bn_name')</th>
        <th width="5%">@lang('variant_type.status')</th>
        <th width="12%">@lang('variant_type.action')</th>
    </tr>
@endsection

@section('content')
    @component('components.breadcrumb')
        @slot('title')@lang('variant_type.index')@endslot
        @slot('create_button')
            <a href="{{route('variant_type.create')}}" class="btn btn-primary btn-sm waves-effect waves-light modal_lg_button">
                <i class="fa fa-plus-circle"></i> @lang('variant_type.create')
            </a>
        @endslot
    @endcomponent
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            <li class="nav-item" role="presentation">
                <button class="nav-link text-primary" id="deleted_btn_tab" data-bs-toggle="tab" data-bs-target="#deleted_list" type="button" role="tab" aria-controls="deleted" aria-selected="false">
                    @lang('variant_type.deleted_list')
                </button>
            </li>
        @endif
    </ul>
    ...
    <script type="text/javascript">
        ...
        @if(\App\Helpers\MenuHelper::CustomElementPermission('deleted_list'))
            $('#deleted_list_datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('variant_type.deleted_list') }}',
                    type: 'get',
                    dataType: 'JSON',
                    cache: false,
                },
